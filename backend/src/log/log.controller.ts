...
import { idSchema, logListSchema } from './log.schemas'
...
export class LogController {
  ...
  async getLogList(@Body(new ZodBodyPipe(logListSchema)) body: any) {
    return { code: 200, data: await this.logService.getLogList(body), msg: 'success' }
  }

  @Post('delete')
  @CheckPermission(PERMISSION_CODES.LOG_DELETE)
  async deleteLog(@Body(new ZodBodyPipe(idSchema)) body: any) {
