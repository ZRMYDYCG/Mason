...
export class CaptchaService {
  ...
  async getGraphicCaptcha() {
    ...
    const id = nanoid()
    await this.storeCaptcha(id, captcha.text, 'graphic')
  ...
  async getArithmeticCaptcha() {
    ...
    const id = nanoid()
    await this.storeCaptcha(id, captcha.text, 'arithmetic')
    return { id, svg: captcha.data }
  }
  ...
  async getSliderCaptcha() {
    ...
    const l = puzzleSize
    const r = tabRadius
    const puzzlePath = `M ${x} ${y} h ${l} v ${l / 2 - r} c ${r},${-2} ${r},${r * 2 + 2} 0,${r * 2} v ${l / 2 - r} h -${l} z`
    const bgSvg = `<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg"><rect width="100%" height="100%" fill="${color}" />${shapes}<path d="${puzzlePath}" fill="rgba(0,0,0,0.5)" stroke="white" stroke-width="2"/></svg>`
    const vbPadding = 2
    const vbWidth = l + r + vbPadding * 2
    const vbHeight = l + vbPadding * 2
    const puzzleSvg = `<svg width="${vbWidth}" height="${vbHeight}" viewBox="${x - vbPadding} ${y - vbPadding} ${vbWidth} ${vbHeight}" xmlns="http://www.w3.org/2000/svg"><defs><clipPath id="cp"><path d="${puzzlePath}" /></clipPath></defs><g clip-path="url(#cp)"><rect x="0" y="0" width="${width}" height="${height}" fill="${color}" />${shapes}</g><path d="${puzzlePath}" stroke="white" stroke-width="2" fill="none"/></svg>`
    return {
      id,
  ...
  async getRotateCaptcha() {
    ...
    await this.storeCaptcha(id, angle.toString(), 'rotate')
    return { id, imgUrl: 'https://picsum.photos/200/200' }
  }
  ...
  async getClickCaptcha() {
    ...
    await this.storeCaptcha(id, JSON.stringify(points), 'click')
    const width = 300
    const height = 200
    const texts = points
    ...
    return {
      id,
      imgUrl: `data:image/svg+xml;base64,${Buffer.from(svg).toString('base64')}`,
