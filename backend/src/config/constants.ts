...
export const getKeyPath = (file: string) => {
  const sourcePath = path.resolve(process.cwd(), 'src', 'config', 'keys', file)
  if (fs.existsSync(sourcePath)) return sourcePath
  return path.resolve(__dirname, 'keys', file)
}

export const readPrivateKey = () => fs.readFileSync(getKeyPath('private-key.pem'))
export const readPublicKey = () => fs.readFileSync(getKeyPath('public-key.pem'))
