...
import { deptCreateSchema, deptUpdateSchema, idSchema } from './department.schemas'
...
export class DepartmentController {
  ...
  async allDepartmentList() {
    return { code: 200, data: await this.departmentService.getAllDepartmentList(), msg: '获取部门列表成功' }
  }

  @Post('add')
  @CheckPermission(PERMISSION_CODES.DEPARTMENT_ADD)
  async addDepartment(@Body(new ZodBodyPipe(deptCreateSchema)) body: any) {
  ...
  async updateDepartment(@Body(new ZodBodyPipe(deptUpdateSchema)) body: any) {
  ...
  async deleteDepartment(@Body(new ZodBodyPipe(idSchema)) body: any) {
    if (!(await this.departmentService.getDeptById(body.id))) throw new AppError(ERROR_TYPES.DEPT_NOT_EXISTS)
