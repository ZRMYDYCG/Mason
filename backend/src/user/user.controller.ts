...
export class UserController {
  ...
  async userList(@Body(new ZodBodyPipe(userListSchema)) body: UserListBody) {
  ...
  async addUser(
    @Body(new ZodBodyPipe(userCreateSchema)) body: UserCreateBody,
    @CurrentUser() current: CurrentUser
  ) {
    if (await this.userService.getUserByName(body.username)) {
      throw new AppError(ERROR_TYPES.USER_ALREADY_EXISTS)
    }
  ...
  async updateProfile(
    @Body(new ZodBodyPipe(userProfileUpdateSchema)) body: UserProfileUpdateBody,
    @UploadedFile() file: Express.Multer.File,
    @CurrentUser() user: CurrentUser
  ) {
    const avatar = file
      ? `http://${process.env.APP_HOST || 'localhost'}:${process.env.APP_PORT || 8000}/public/uploads/${file.filename}`
      : undefined
  ...
  async updateUser(
    @Body(new ZodBodyPipe(userUpdateSchema)) body: UserUpdateBody,
    @CurrentUser() current: CurrentUser
  ) {
    ...
    if (existing && Number(existing.id) !== body.id) {
      throw new AppError(ERROR_TYPES.USER_ALREADY_EXISTS)
    }
  ...
  async deleteUser(
    @Body(new ZodBodyPipe(idSchema)) body: IdBody,
    @CurrentUser() current: CurrentUser
  ) {
