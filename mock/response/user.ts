...
export const getUser = (username: string): { username: string, role: Array<string> } => {
    return {
        username,
        role: user_role.filter(item => item.userName === username).map(item => item.roleName)
    }
}

export const getRoute = (username: string): IMenubarList[] => {
    const { role } = getUser(username)
    const arr = role_route.filter(item => role.findIndex(i => i === item.roleName)!== -1)
